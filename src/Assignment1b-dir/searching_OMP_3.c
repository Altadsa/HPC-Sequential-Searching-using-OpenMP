...
int hostMatch(long *comparisons, int thread_count)
{
	...
	// set pattern location to end of text such that we work our way back through the text to find the earliest location
	volatile int patternLoc = textLength+1;

	int breakSearch = 0;

	int halfLength = patternLength / 2;
	#pragma omp parallel for reduction(+: localComparisons) reduction(+: localPatterns) shared(patternLoc) private(j, k) firstprivate(breakSearch) num_threads(thread_count)
	for (i = 0; i <= lastI; i++)
	{
        k = i;
        j = 0;
        while (j < halfLength && breakSearch == 0)
        {
            localComparisons+=2;
            if (textData[k] == patternData[j] && textData[k+(patternLength-j)-1] == patternData[j+(patternLength-j)-1])
            {
                k++;
                j++;
            }
