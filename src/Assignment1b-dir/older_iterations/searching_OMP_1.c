...
int hostMatch(long *comparisons, int thread_count)
{
	...
	volatile int breakSearch = 0;

	#pragma omp parallel for reduction(+: localComparisons) shared(patternLoc, breakSearch) private(i, j) schedule(dynamic) num_threads(thread_count)
	for (i = 0; i <= lastI; i++)
	{
		if (breakSearch != 1)
		{
			j = 0;
			while (j < patternLength && patternLoc == -1 && textData[i+j] == patternData[j])
			{
				localComparisons++;
				j++;
			}
